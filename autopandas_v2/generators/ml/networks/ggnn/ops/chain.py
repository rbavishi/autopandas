...
class ModelChain(BaseSmartGenGGNN):
    def preprocess_data(self, path, is_training_data=False):
        ...
        self.params['num_options'] = num_options
        reader.close()

    def process_raw_graph(self, graph):
        processed = super().process_raw_graph(graph)
        processed['picked'] = graph.get('picked', -1)
        return processed

    def define_placeholders(self):
        super().define_placeholders()
        self.placeholders['picked'] = tf.placeholder(tf.int64, [None], name='picked')
    ...
    def final_predictor(self):
        ...
        return utils.MLP(self.params['hidden_size_node'], self.params['num_options'],
    ...
    def make_model(self, mode):
        ...
        with tf.variable_scope("out_layer"):
            ...
            labels = self.placeholders['picked']
            ...
            top_k = tf.nn.top_k(probabilities, k=self.params['num_options'])
            self.ops['preds'] = top_k.indices
            self.ops['probs'] = top_k.values
            self.ops['targets'] = self.placeholders['picked']
    ...
    def save_interface(self, path: str):
        from autopandas_v2.generators.ml.networks.ggnn.interfaces import ModelChainInterface
