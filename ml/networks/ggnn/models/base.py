...
from autopandas_v2.utils.ioutils import IndexedFileReader, IndexedFileWriter
...
class BaseGGNN(ABC):
    ...
    def build_graph_model(self, mode='training', restore_file=None):
        ...
        with self.graph.as_default():
            if self.params.args.get('device', None) is not None:
                with tf.device(self.params.args.device):
                    setup_graph()
            else:
                setup_graph()
    ...
    def run_training(self):
        ...
        if self.params.args.get('restore_params', None) is not None:
            old_args = self.params.args
            with open(self.params.args.restore_params, 'rb') as f:
                self.params.update(pickle.load(f))

            self.params.args = old_args
        ...
        #  Setup the model
        self.build_graph_model(mode='training', restore_file=self.params.args.get('restore_file', None))
    ...
    def infer(self, raw_graph_data, **kwargs):
        ...
        for step, batch_data in enumerate(batch_iterator):
            ...
            for p, v in zip(result[0], result[1]):
                preds.append(list(map(lambda x: (mapping[x[0]], x[1]), zip(p, v))))
